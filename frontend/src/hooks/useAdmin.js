...
export function useAdmin() {
  ...
  const createUser = useCallback(async (userData) => {
    try {
      ...
      const { data } = await api.post("/admin/users", backendPayload);
  ...
  const updateUser = useCallback(async (id, userData) => {
    try {
      ...
      const { data } = await api.put(`/admin/users/${id}`, backendPayload);
      await loadUsers(); // Refresh the list
      return { success: true, data };
    } catch (err) {
  ...
  const resetPassword = useCallback(async (id, newPassword) => {
    try {
      setLoading(true);
      const { data } = await api.put(`/admin/users/${id}/password`, {
  ...
  return useMemo(
    ...
    [loading, error, users, loadUsers, createUser, updateUser, deleteUser, toggleUserStatus, resetPassword]
  );
}
