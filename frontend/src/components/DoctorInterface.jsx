...
export function DoctorInterface({ patients: unused, onUpdatePatient, onMoveToStage, getTotalTime, currentDoctorUsername }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const prof = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/${encodeURIComponent(currentDoctorUsername)}`)
      ...
      try {
        const list = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/${encodeURIComponent(currentDoctorUsername)}/patients`)
          .then(r => r.ok ? r.json() : []);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        setIcdLoading(true);
        const res = await fetch(`https://node-mysql-api-zsam.onrender.com/api/icd10/search?q=${encodeURIComponent(icdSearchTerm)}`, {
...
const completedPatients = useMemo(() =>
  ...
  const startConsult = async () => {
    ...
    try {
      const res = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/encounters/${selectedPatient}/start`, {
        method: "POST",
  ...
  const completeConsult = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/encounters/${selectedPatient}/complete`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const resumeObservation = async (patientId) => {
    try {
      const res = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/encounters/${patientId}/resume`, {
        method: "POST",
        headers: { "Content-Type": "application/json" }
      });
  ...
  const markAdmittedAsDeparted = async (patientId) => {
    try {
      const res = await fetch(`https://node-mysql-api-zsam.onrender.com/api/doctor/encounters/${patientId}/depart`, {
        method: "POST",
        headers: { "Content-Type": "application/json" }
      });
