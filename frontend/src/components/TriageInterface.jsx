...
export function TriageInterface({ onBack }) {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        setLoading(true);
        const res = await fetch("https://cmsc128-backend.onrender.com/api/triage");
                  ...
                  {[1, 2, 3, 4, 5].map((level) => (
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          await fetch(
                            `https://cmsc128-backend.onrender.com/api/triage/encounters/${selectedPatient.encounter_id}/triage`,
                            {
                              method: "POST",
